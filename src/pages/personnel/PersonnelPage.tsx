...
export default function PersonnelPage() {
  ...
  const { isAdmin, role, unitId } = useAuth();
  ...
  const [designations, setDesignations] = useState<
    { id: string; unit_classification: number; name: string }[]
  >([]);
  ...
  }, [editingPerson, unitId]);

  useEffect(() => {
    fetchData();
    fetchDesignations();
  }, [activeTab, unitId]); // Added unitId dependency

  // Fetch designations when unit selection changes
  useEffect(() => {
    fetchDesignations();
  }, [selectedUnit]);

  // Filter designations based on selected unit's classification
  useEffect(() => {
    ...
    // Find the selected unit to get its classification
    ...
    // Filter designations by unit classification
    // Note: designation table has 'unit_classification' (with typo) column
    const filtered = designations.filter(
      (designation) =>
        designation.unit_classification === selectedUnitObj.classification,
    );
    setFilteredDesignations(filtered);
  }, [designations, selectedUnit, units]);
  ...
  const fetchData = async () => {
    ...
    try {
      const [personnelData, unitsRes, rankRes] = await Promise.all([
        getAllPersonnel(),
      ...
      let personnel = personnelData;

      // Filter by approval
      if (activeTab === "pending") {
        personnel = personnel.filter((p) => !p.is_approved);
      }

      // Filter by unit
      if (!isAdmin && unitId) {
        personnel = personnel.filter((p) => p.unit_id === unitId);
      }

      const sortedPersonnel = [...personnel].sort((a, b) => {
        // 1. Unit
        const unitLevelA = a.unit?.level ?? -Infinity;
        const unitLevelB = b.unit?.level ?? -Infinity;

        // Higher level first
        if (unitLevelA !== unitLevelB) {
          return unitLevelB - unitLevelA;
        }

        // 2. Rank (highest first)
        ...
        // 3. Badge Number (ascending)
        return (a.badge_number ?? "").localeCompare(
          b.badge_number ?? "",
          undefined,
          {
            numeric: true,
  ...
  const fetchDesignations = async () => {
    try {
      ...
      if (data) {
        ...
        // Update filtered designations based on current unit selection
        if (!selectedUnit || selectedUnit === "all") {
          ...
          if (selectedUnitObj) {
            ...
            const filtered = data.filter(
              (designation) =>
                designation.unit_classification ===
                selectedUnitObj.classification,
            );
            setFilteredDesignations(filtered);
          } else {
            setFilteredDesignations([]);
  ...
  const filteredPersonnel = personnel.filter((p) => {
    // 1. Safe, multi-field search logic
    const matchesSearch = (() => {
      const query = searchQuery.toLowerCase();
      if ((p.fullname || "").toLowerCase().includes(query)) return true;
    ...
    // 2. Keep your existing unit filter exactly as it was
  ...
  const handleApprove = async (id: string) => {
    if (
      !window.confirm(
        "Are you sure you want to approved this officer? This will allow the user to log in into the website and the mobile app.",
      )
    )
      return;
  ...
  const handleAddPersonnel = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    setFormError(null);
    setFormSuccess(null);
            ...
            <div className="flex bg-[var(--secondary)]/[0.1] dark:bg-[var(--secondary)]/[0.2] p-1 rounded-xl">
              <button
                ...
                className={`px-4 py-1.5 rounded-lg font-blacktracking-tighter transition-all ${
                  activeTab === "all"
                    ? "bg-[var(--primary)]/[0.9] dark:bg-slate-900 text-white shadow-sm"
                    : "text-black dark:text-[var(--text)]/[0.6] hover:text-[var(--text)]/[0.7] dark:hover:text-[var(--text)]/[0.4]"
                }`}
              >
                All Directory
              </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <input
                ...
                className="bg-slate-50 dark:bg-slate-800 border rounded-xl p-3 font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 outline-none"
              />
            </div>
            ...
            <div className="relative">
              <select
                ...
                disabled={!isAdmin}
                className={`bg-slate-50 dark:bg-slate-800 border rounded-xl p-3 font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 outline-none ${!isAdmin ? "opacity-50 cursor-not-allowed" : ""}`}
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            ...
            >
              {/* Form Alerts */}
              {formError && (
                <div className="bg-red-50 dark:bg-red-900 border border-red-100 dark:border-red-900/30 text-red-600 dark:text-red-400 p-4 rounded-xl flex items-start gap-3 mb-4">
                  <AlertCircle className="w-5 h-5 shrink-0 mt-0.5" />
                  <p className="font-semibold">{formError}</p>
                </div>
              )}
              {formSuccess && (
                <div className="bg-green-50 dark:bg-green-900 border border-green-100 dark:border-green-900/30 text-green-600 dark:text-green-400 p-4 rounded-xl flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!isAdmin}
                  className={`w-full mt-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-3 px-4 font-bold outline-none focus:ring-2 focus:ring-blue-500/20 transition-all ${!isAdmin ? "opacity-50 cursor-not-allowed" : ""}`}
                >
