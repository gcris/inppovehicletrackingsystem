...
export default function SchedulePage() {
  ...
  const handleAddSchedule = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    setError(null);
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-slate-800 dark:text-slate-200">
                            {assignments?.map((assigned) => {
                              return (
                                <div
                                  key={assigned.id || assigned.personnel?.id}
                                  className="block text-md"
                                >
                                  {assigned.personnel?.rank?.rank_name}{" "}
                                  {assigned.personnel?.fullname ||
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 overflow-y-auto max-h-[80vh]">
              <form onSubmit={handleAddSchedule} className="p-6 space-y-5">
                {error && (
                  <div className="bg-red-50 dark:bg-red-900 border border-red-100 dark:border-red-900/30 text-red-600 dark:text-red-400 p-4 rounded-xl flex items-start gap-3">
