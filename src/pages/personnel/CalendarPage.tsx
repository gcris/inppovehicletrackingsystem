...
export default function CalendarPage() {
  ...
  const fetchCalendars = async () => {
    ...
    try {
      ...
      if (!isAdmin && unitId) {
        // Show events where distributions array contains the current unit ID
        calendarsQuery = calendarsQuery.contains("distributions", [unitId]);
      } else if (isAdmin && unitFilter) {
        // Admin viewing specific unit: show events tagged with that unit
        calendarsQuery = calendarsQuery.contains("distributions", [unitFilter]);
      }
      // Admin viewing all units: no distributions filter (show all events)
  ...
  const fetchUnits = async () => {
    try {
      ...
      const { data: distribution } = await unitQuery;
  ...
  const handleAddEditCalendar = async (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="px-2 py-0.5 rounded text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200">
                            {event.distributions
                              ?.map((unitId: string) => {
                                const unit = distributionList.find(
                                  (u) => u.id === unitId,
                                );
                                return unit ? unit.unit_name : "Unknown Unit";
                              })
                              .join(", ") || "No units tagged"}
                          </span>
                        </div>
                        {event.description && (
                ...
                <form onSubmit={handleUpdate} className="p-8 space-y-5">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const selectedUnits = Array.from(e.target.selectedOptions)
                          .map(option => option.value);
                        setFormData({ ...formData, distributions: selectedUnits });
                      }}
                      className="w-full mt-1.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl py-3 px-4 font-bold outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                    >
                      <option value="">Select Units to Tag</option>
                      {unitList.map((unit) => (
                        <option key={unit.id} value={unit.id}>
                          {unit.unit_name}
                        >
                    ...
                    {formData.distributions.length > 0 && (
                      <p className="mt-2 text-slate-600 dark:text-slate-400 text-sm">
                        Tagged to {formData.distributions.length} unit(s)
                      </p>
                    )}
                  </div>
